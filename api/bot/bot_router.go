...
var (
	insBotRouter = BotRouters{
		List: map[string]telebot.IHandler{
			...
			// OnClickRouter group
			OnClickRouters:      telebot.HandlerFunc(Event.OnClickRouters),
			OnClickAddRouter:    AddResolverConversation(OnClickAddRouter, "/cancel"),
			OnClickUpdateRouter: UpdateResolverConversation(OnClickUpdateRouter, "/cancel"),
			OnClickDetailRouter: telebot.HandlerFunc(Event.OnClickDetailRouter),
			OnClickDelRouter:    telebot.HandlerFunc(Event.OnClickDelRouter),
...
type BotRouters struct {
	List map[string]telebot.IHandler
	Btns map[*telebot.Btn]telebot.IHandler
	Test map[string]telebot.IHandler
}

func Router() *BotRouters {
	return &insBotRouter
}
